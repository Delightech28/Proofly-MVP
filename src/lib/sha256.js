// Lightweight SHA-256 implementation with WebCrypto fallback.
// Exports an async function sha256Hex(text) that returns hex digest.
export async function sha256Hex(message) {
  ...
  // Adapted compact implementation of SHA-256 (no external deps)
  ...
  // Pre-processing: append 0x80, then pad with zeros, then append length
  const withOne = new Uint8Array(msgBytes.length + 1);
  withOne.set(msgBytes);
  withOne[msgBytes.length] = 0x80;

  // Calculate new length in bytes: multiple of 64 minus 8 for length
  ...
  for (let i = 0; i < 8; i++) {
    padded[padded.length - 1 - i] = (l >>> (i * 8)) & 0xff;
  }
  ...
  const hex = [h0, h1, h2, h3, h4, h5, h6, h7].map(h => h.toString(16).padStart(8, '0')).join('');
  return hex;
}
